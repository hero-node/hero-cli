...
- (CGPoint)randomDestinationX:(CGFloat)x Y:(CGFloat)y imageSize:(CGSize)size {
	CGPoint destination;
		destination.x=MIN(size.width,(CGFloat)(random()%40) +x);
		destination.y=MIN(size.height,(CGFloat)(random()%40) +y);
	return destination;
}
-(void)rippleEffectWithDuration:(NSTimeInterval)duration
{
...
-(void)cubeWithDuration:(NSTimeInterval)duration
{
    CATransition *animation = [CATransition animation];
    animation.delegate = self;
    animation.duration = duration;
	...
	animation.fillMode = kCAFillModeForwards;
	animation.removedOnCompletion = NO;
    animation.type = @"cube";//110
    [self.layer addAnimation:animation forKey:@"animation"];
}
-(void)suckEffectWithDuration:(NSTimeInterval)duration
{
    CATransition *animation = [CATransition animation];
    animation.delegate = self;
    animation.duration = duration;
...
-(void)oglFlipWithDuration:(NSTimeInterval)duration
{
    CATransition *animation = [CATransition animation];
    animation.delegate = self;
    animation.duration = duration;
	...
	animation.fillMode = kCAFillModeForwards;
	animation.removedOnCompletion = NO;
    animation.type = @"oglFlip";//110
    [self.layer addAnimation:animation forKey:@"animation"];
}

-(void)alphaChangeWithtimes:(NSInteger)times
{
...
-(void)bounce
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        BOOL growDir = 0;
        if (self.frame.size.width/self.bounds.size.width >= 1.400000) {
            growDir = 0;
        }
        if (self.frame.size.width/self.bounds.size.width <= 0.800000) {
            growDir = 1;
        }
        switch (growDir) {
            case 0:
                ...
                break;
            case 1:
                if(self.frame.size.width/self.bounds.size.width -1.0 < 0.1)
                {
...
-(void)shake
{
    int centerX = ((UIView*)self).center.x;
    [UIView animateWithDuration:0.1 animations:^{
        ((UIView*)self).center = CGPointMake(centerX + 8, ((UIView*)self).center.y);
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.1 animations:^{
            ((UIView*)self).center = CGPointMake(centerX - 7, ((UIView*)self).center.y);
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.1 animations:^{
                ((UIView*)self).center = CGPointMake(centerX + 6, ((UIView*)self).center.y);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:0.1 animations:^{
                    ((UIView*)self).center = CGPointMake(centerX - 5, ((UIView*)self).center.y);
                } completion:^(BOOL finished) {
                    [UIView animateWithDuration:0.1 animations:^{
                        ((UIView*)self).center = CGPointMake(centerX + 4, ((UIView*)self).center.y);
                    } completion:^(BOOL finished) {
                        [UIView animateWithDuration:0.1 animations:^{
                            ((UIView*)self).center = CGPointMake(centerX - 3, ((UIView*)self).center.y);
                        } completion:^(BOOL finished) {
                            [UIView animateWithDuration:0.1 animations:^{
                                ((UIView*)self).center = CGPointMake(centerX + 2, ((UIView*)self).center.y);
                            } completion:^(BOOL finished) {
                                [UIView animateWithDuration:0.1 animations:^{
                                    ((UIView*)self).center = CGPointMake(centerX -1, ((UIView*)self).center.y);
                                } completion:^(BOOL finished) {
                                    [UIView animateWithDuration:0.1 animations:^{
                                        ((UIView*)self).center = CGPointMake(centerX + 0, ((UIView*)self).center.y);
                                    } completion:^(BOOL finished) {
                                        ;
                                    }];
                                }];
...
-(void)glassBrokenWithDuration:(NSTimeInterval)duration completion:(void (^)())completion
{
    UIImage *selfImage = [self screenshot];
    for (UIView *view in self.subviews) {
        [view removeFromSuperview];
    }
    for(int x=0;x<self.bounds.size.width;x+=5)
    {
        for(int y=0;y<self.bounds.size.height;y+=5)
        {
            CGRect frame=CGRectMake(x, y, 5, 5);
            CALayer *layer=[CALayer layer];
            layer.frame=frame;
            CGImageRef subimage=CGImageCreateWithImageInRect(selfImage.CGImage, frame);
            ...
            CGPoint dest = [self randomDestinationX:x Y:y imageSize:self.bounds.size];
            ...
            FlipAnimation.toValue= [NSValue valueWithCATransform3D:CATransform3DMakeScale(0.3, 0.3, 0.3)];
            FlipAnimation.duration=duration;
            FlipAnimation.fillMode=kCAFillModeForwards;
            FlipAnimation.removedOnCompletion= NO;
            group.animations = [NSArray arrayWithObjects:opacity, position, nil];
            [group setValue:@"glass" forKey:@"type"];
            [layer addAnimation:group forKey:@"animation"];
            [self.layer addSublayer:layer];
        }
    }
    double delayInSeconds = duration - 0.2;
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        for (CALayer *view in self.layer.sublayers ) {
            view.hidden = YES;
        }
        completion();
    });
...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (BCAxis) axis
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        layer.contents = (id)(sliceImage.CGImage);

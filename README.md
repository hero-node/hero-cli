...
Run `npm start` and then open [http://localhost:3000/?state=http://localhost:3000/pages/login.html](http://localhost:3000/?state=http://localhost:3000/pages/login.html) to see your app.<br>
...
You may curious about where is the `pages/login.html`. Yes, it's generated by hero-cli. See [Generate HTML](#generate-html)
...
Below JavaScript file `src/pages/login.js` will generate a HTML file access by URL `/pages/login.html`, that's why we can visit [http://localhost:3000/pages/login.html](http://localhost:3000/pages/login.html).

```javascript
// content of file: src/pages/login.js
import { Entry } from 'hero-cli/decorator';

// class marked by @Entry
// will generate HTML accessed by URL /pages/login.html
// Equal to
// @Entry({
...
```
Then you can access the `login.html` by URL `/mkt/pages/login.html`

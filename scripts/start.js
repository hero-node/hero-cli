...
let formatWebpackMessages = require('../lib/formatWebpackMessages')
...
let updateEntryFile = require('../lib/updateWebpackEntry')
let commandName = Object.keys(pgk.bin)[0]
let __heroContentBaseURL = '/__hero_app_contentBase_' + new Date().getTime()
...
let customPort = yargs.argv.p
// We attempt to use the default port but if it is busy, we offer the user to
// run on a different port. `detect()` Promise resolves to the next free port.
let DEFAULT_PORT = (customPort && /^\d+$/.test(customPort)) ? customPort : heroCliConfig.devServerPort
...
let expectedJsType = /\.js$/
// Something to use when events are received.
let needUpdateEntry = false
let watcher
...
let checkRebuild = _.throttle(_checkRebuild, 1000, { 'trailing': true })

let fileAddWatchListener = function(path) {
  // console.log('File ADD: ' + path);
  watcher.add(path)
  checkRebuild(path)
}

let fileChangeWatchListener = function(path) {
  // console.log('File Change: ' + path);
  // console.log(watcher.getWatched());
  checkRebuild(path)
}

let fileRemoveWatchListener = function(path) {
  // console.log('File REMOVE: ' + path);
  watcher.unwatch(path)
  checkRebuild(path, true)
}

let addDirListener = function(path) {
  // console.log('Dir ADD: ' + path);
  watcher.add(path)
}

let unlinkDirListener = function(path) {
      ...
      rewrites: [
        {
          from: /^\/.*$/,
          to: function(context) {
            let isServeRoot = (config.output.publicPath === '/' || homePageConfig.isRelativePath)
            let subPath = isServeRoot ? context.parsedUrl.pathname : context.parsedUrl.pathname.slice(config.output.publicPath.length - 1)

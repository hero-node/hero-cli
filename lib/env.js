'use strict';

var paths = require('./paths');
var path = require('path');
var checkRequiredFiles = require('./checkRequiredFiles');
var chalk = require('chalk');
var config = require('../config/hero-config.json');

// Warn and crash if required files are missing
if (!checkRequiredFiles([paths.heroCliConfig, paths.appIndexJs])) {
    process.exit(1);
}

function getClientEnvironment(env) {
    var heroFileConfig = require(paths.heroCliConfig);
    var environments = heroFileConfig[config.environmentKey];

    if (environments && Object.keys(environments) === 1) {
        if (!env) {
            env = Object.keys(environments)[0];
        }
    }
    if (env) {
        if (!environments || !environments[env]) {
            console.log('Unknown Environment "' + env + '".');
            console.log('You need to update ' + paths.heroCliConfig);
            console.log(chalk.red('  Add attribute "' + config.environmentKey + '" in: ') + chalk.cyan(paths.heroCliConfig) + ' and specify the config file path');
            process.exit(1);
        }
    }

    var configPath = path.resolve(paths.heroCliConfig, '../', heroFileConfig[config.environmentKey][env]);
    var raw = require(configPath);
  // Stringify all values so we can feed into Webpack DefinePlugin
    var stringified = {
        'process.env': JSON.stringify(raw)
    };

    console.log(stringified);
    return { raw, stringified };
}

module.exports = getClientEnvironment;
